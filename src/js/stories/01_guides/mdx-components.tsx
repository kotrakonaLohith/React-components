...
const components: { [s: string]: FC<Props> } = {
  ...
  h2: ({ children }) => {
    return (
      <h2 className="heading-secondary mb-32 text-dark-blue-400">
        {children}
      </h2>
    )
  },
  h3: ({ children }) => {
    return <h3 className="heading-tertiary mb-32">{children}</h3>
  },
  h4: ({ children }) => {
    return <h4 className="heading-alt text-xs mb-32 border-l-3 border-bright-blue-500 pl-8">{children}</h4>
  },
  h5: ({ children }) => {
